...
-- Obter os IDs das empresas e criar perfis
DO $$
DECLARE
    dmm_id uuid;
    arseg_id uuid;
...
BEGIN
    ...
    -- Criar perfis dos clientes (usando cliente_admin)
    INSERT INTO public.perfis (email, nome_completo, funcao, empresa_id) VALUES
    ('arseg@arsegconsultoria.com.br', 'Arseg - Adalto, Bruna, Jeferson, Thaina, Pablo', 'cliente_admin', arseg_id),
    ('embelleze@embelleze.com.br', 'Embelleze - Daniele, Nara', 'cliente_admin', embelleze_id),
    ('limpmax@papeislimpmax.com.br', 'LimpMax - Leonardo, Lúcia', 'cliente_admin', limpmax_id),
    ('dolbclean@dolbclean.com.br', 'Dolb Clean - Renato, Adriano', 'cliente_admin', dolb_id),
    ('knnaraucaria@knn.com.br', 'Knn Araucária - Eduardo', 'cliente_admin', knn_id)
    ON CONFLICT (email) DO NOTHING;
END
$$;